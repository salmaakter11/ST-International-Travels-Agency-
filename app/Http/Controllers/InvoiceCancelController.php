...
class InvoiceCancelController extends Controller
{
    public function cancel_list()
    {
        ...
        return view('dashboard.invoice.cancel_list');

    }

    public function pending_cancel($id){
        ...
        if ($invoice) {
            return redirect()->back()->with('success', 'Cancel Successfully Complete');
        } else {
            return redirect()->back()->with('error', 'Cancel Faield');
        }
    }

    public function cancel_list_show()
    {
    ...
    public function cancel_search_print(Request $request){
        $from_date = isset($request->from_date) ? date("Y-m-d", strtotime($request->from_date)) : '';
        $to_date = isset($request->to_date) ? date("Y-m-d", strtotime($request->to_date)) : '';
        $status= 4;
