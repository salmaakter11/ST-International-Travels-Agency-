...
class InvoiceController extends Controller
{
    public function pending_list()
    {
        ...
        return view('dashboard.invoice.pending_list');

    }
    public function pending_list_show()
    {
    ...
    public function pending_process($id)
    {
        ...
        if ($invoice) {
            return redirect()->back()->with('success', 'Processing Successfully Complete');
        } else {
            return redirect()->back()->with('error', 'Processing Faield');
        }
    ...
    public function pending_search_print(Request $request){
        $from_date = isset($request->from_date) ? date("Y-m-d", strtotime($request->from_date)) : '';
        $to_date = isset($request->to_date) ? date("Y-m-d", strtotime($request->to_date)) : '';
        $status= 0;
        ...
        // return $invoice;
        $pdf = Pdf::loadView('dashboard.invoice.client_list_print2', compact('invoice','status','from_date','to_date'));
